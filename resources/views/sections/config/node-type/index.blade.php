...
@section('content-subtitle', 'Tipos de Nodos')

@section('breadcrumb')
    <li><a href="{{ route('config') }}">Configuración</a></li>
    <li class="active">Tipos de Nodos</li>
@endsection
                ...
                <div class="box-body">
                    <div id="toolbar" class="btn-group">
                        <button data-toggle="modal" data-target="#modal-create" class="btn btn-success"><i
                                    class="fa fa-plus"></i> Nuevo Tipo de Nodo
                        </button>
                    </div>
                    <table
                            id="table"
            ...
            <form method="post" id="form-create">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Nuevo Tipo de Nodo</h4>
                    </div>
            ...
            <form method="post" id="form-edit">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">×</span></button>
                        <h4 class="modal-title">Editar Tipo de Nodo</h4>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
    ...
    <script>
        ...
        $(function () {
            $('#form-edit').submit(function (e) {
                ...
                e.preventDefault();
                var url = '{{route('node-types.update')}}';
                $.ajax({
                    type: 'POST',
                    url: url,
                    data: dataForm,
                    success: function (data) {
                        ...
                        if(data.status === 'success'){
                            showToastSuccess(data.message);
                            showToastWarning('Tenga en cuenta que el tipo de nodo que ha modificado puede estar siendo utilizado por nodos y este cambio será reflejado en estos.');
        ...
        function changeStatusToggle(rowId){
            ...
            $.ajax({
                type: 'POST',
                url: '{{route('node-types.change-status')}}',
                data: {
                    _token : '{{ csrf_token() }}',
                    id : item.id,
                    active : itemStatus
                ...
                success: function (data) {
                    ...
                    if(data.status === 'success'){
                        ...
                        showToastSuccess('Se ha ' + status + ' el tipo de nodo ' + data.entity.name);
                        if(status == 'desactivado'){
                            showToastWarning('Tenga en cuenta que el tipo de nodo que ha deshabilitado puede estar siendo utilizado por nodos y este dejará de aparecer como una opción seleccionable.');
                        }
                        updateRow(items.findIndex(item => item.id == data.entity.id), data.entity);
                    }
                },
        ...
        function remove(id){
            ...
            swal({
                title: '¿Estas seguro?',
                text: "Si eliminas este tipo de nodo se perderán todas las relaciones actuales.",
                type: 'question',
                ...
                if (result.value) {
                    ...
                    var url = '{{route('node-types.destroy')}}';
                    $.ajax({
                        type: 'POST',
                        url: url,
                        data: {
                        ...
                        success: function (data) {
                            ...
                            if(data.status === 'success'){
                                showToastSuccess(data.message);
                                showToastWarning('Tenga en cuenta que el tipo de nodo que ha eliminado ya no puede ser referenciado.');
                                $('#table').bootstrapTable('refresh');
                            }
                        },
                        error: function (error) {
